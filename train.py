import os
# os.environ['OMP_NUM_THREADS'] = '1' # 线程的数量限制为单线程执行,window使用多线程会报错
import argparse
import torch
...
def get_args():
    ...
    parser.add_argument("--world", type=int, default=1)   # {1, 2, 3, 4, 5, 6, 7, 8}
    parser.add_argument("--stage", type=int, default=1)   # {1, 2, 3, 4} 关卡
    ...
    parser.add_argument("--log_path", type=str, default="tensorboard/a3c_super_mario_bros")  # tensorboard文件夹
    parser.add_argument("--saved_path", type=str, default="trained_models")  # 保存的模型文件夹
...
def train(opt):
    ...
    if opt.load_from_previous_stage:  # 加载上个关卡的模型权重
        if opt.stage == 1:
